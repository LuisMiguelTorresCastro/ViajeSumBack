...
router.post('/', async (req, res) => {
  ...
  const redesAgencia = req.body.redesAgencia !== undefined ? req.body.redesAgencia.trim() : null;
  ...
  const sql = `
    INSERT INTO Agencia (nombreAgencia, descripcion, imageUrl, redesAgencia, correo, telefono, direccion) 
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `;

  try {
    await query(sql, [nombreAgencia, descripcion, imageUrl, redesAgencia, correo, telefono, direccion]);
...
router.patch('/:id', async (req, res) => {
  ...
  console.log('Datos de la agencia:', { nombreAgencia, descripcion, imageUrl, redesAgencia, correo, telefono, direccion });

  try {
    await query(`UPDATE Agencia SET nombreAgencia = ?, descripcion = ?, imageUrl = ?, redesAgencia = ?, correo = ?, telefono = ?, direccion = ?
      WHERE idAgencia = ?`, [nombreAgencia, descripcion, imageUrl, redesAgencia, correo, telefono, direccion, idInt]);
